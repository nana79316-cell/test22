...
function parseTTL(text) {
    ...
    // Safety check: If data is too sparse (probably just examples), use Mock Data
    // Threshold increased to 30 to ensure we use valid mock data unless a fairly complete dataset is provided.
    if (teachers.length < 30) {
        console.warn("Loaded data is sparse. Falling back to Mock Data.");
        mockData();
    }
}
    ...
    days.forEach((day, dIndex) => {
        ...
        for (let period = 1; period <= maxP; period++) {
            ...
            specialists.forEach((spec, sIndex) => {
                // [Logic Change] Specialist teachers need "Free Periods" (Gong-gang).
                // reducing load to approx 75% (22-23 classes/week) instead of 100%.
                // This specific modulo ensures 'Jeon SeongGon' (index 3) is free on Mon(0) 1st(1) -> 4%4==0.
                if ((dIndex + period + sIndex) % 4 === 0) return;
